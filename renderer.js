...
const renderInventory = (inventory) => {
  ...
  for(item in inventory.contents){
    ...
    if(inventory.contents[item].reforge === ""){
      document.querySelector("#bag").innerHTML += `<div id="item" class="tooltip" data-pos='${item}' data-item='${JSON.stringify(inventory.contents[item])}'><img src='${inventory.contents[item].img}' height="100%" width="100%"><span class="tooltiptext" style="color:${rarities[inventory.contents[item].rarity]};">${inventory.contents[item].getName()}<br><br><span style="color:white;">Damage: <span style="color:red;">+${inventory.contents[item].damage}</span></span><br><span style="color:white;">Strength: <span style="color:orange;">+${inventory.contents[item].strength}</span></span></span></div>`;
    }else{
      document.querySelector("#bag").innerHTML += `<div id="item" class="tooltip" data-pos='${item}' data-item='${JSON.stringify(inventory.contents[item])}'><img src='${inventory.contents[item].img}' height="100%" width="100%"><span class="tooltiptext" style="color:${rarities[inventory.contents[item].rarity]};">${inventory.contents[item].getName()}<br><br><span style="color:white;">Damage: <span style="color:red;">+${inventory.contents[item].damage}</span><span style="color:#f7ea57;"> (+${inventory.contents[item].dmgBuff})</span></span><br><span style="color:white;">Strength: <span style="color:orange;">+${inventory.contents[item].strength}</span><span style="color:#f7ea57;"> (+${inventory.contents[item].strBuff})</span></span></span></div>`;
    }
  }
  return true;
};
